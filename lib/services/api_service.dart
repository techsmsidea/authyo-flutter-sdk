...
class _ApiService {
  ...
  _ApiService({required String clientId, required String clientSecret, Duration? connectTimeout, Duration? receiveTimeout, Duration? sendTimeout})
    : _dio = Dio(
        BaseOptions(
  ...
  // GET request
  Future<AuthyoResponseModel> get(String url, {Map<String, dynamic>? queryParameters}) async {
    try {
      final response = await _dio.get(url, queryParameters: queryParameters);
      AuthyoResponseModel baseResponse = AuthyoResponseModel.fromJson(response.data);
      if (baseResponse.success == true) {
        return baseResponse;
      } else {
        return AuthyoResponseModel.withError({"error": baseResponse.message ?? "Something went wrong while processing your request", "errorCode": "500"});
      }
    } on DioException catch (e) {
      AuthyoError authyoError = _handleDioError(e);
      return AuthyoResponseModel.withError({"error": authyoError.message, "errorCode": "${authyoError.code}"});
    } catch (e) {
  ...
  // POST request
  Future<AuthyoResponseModel> post(String url, {Map<String, dynamic>? body}) async {
    try {
  ...
  AuthyoError _handleDioError(DioException e) {
    if (e.type == DioExceptionType.connectionTimeout || e.type == DioExceptionType.sendTimeout || e.type == DioExceptionType.receiveTimeout) {
      ...
      if (statusCode >= 500) {
        return InternalServerError('Server error: ${e.message}', code: statusCode);
      } else if (statusCode >= 400) {
        return handleHttpError(statusCode, "${e.message}");
      }
    } else if (e.type == DioExceptionType.connectionError) {
      return ServiceUnavailableError('Network Unavailable: There is no internet connection, please check and try again');
    }
